...
fn main() {
	...
	//C L,4,L,4,L,6,
	//A L,6,R,12,L,6,L,8,L,8
	let func_a = "L,6,R,12,L,6,L,8,L,8";
	let func_b = "L,6,R,12,R,8,L,8";
	let func_c = "L,4,L,4,L,6";
	let func_order = "A,A,B,C,B,A,C,B,C,A";

	for input in func_order.chars() {
		let val = input as u8 as i64;
		inputs.push_back(val);
	}
	inputs.push_back(10);
	for input in func_a.chars() {
		let val = input as u8 as i64;
		inputs.push_back(val);
	}
	inputs.push_back(10);
	for input in func_b.chars() {
		let val = input as u8 as i64;
		inputs.push_back(val);
	}
	inputs.push_back(10);
	for input in func_c.chars() {
		let val = input as u8 as i64;
		inputs.push_back(val);
	}
	inputs.push_back(10);
	...
	let status = execute_program(&mut program_state, &mut inputs, &mut outputs);
	println!("{:?}", status);
	println!("{:?}", outputs);
}
