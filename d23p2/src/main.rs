...
fn main() {
	...
	let mut nat_received: (i64, i64) = (0, 0);
	let mut nat_last_y_sent: Option<i64> = None;
	...
	loop {
		...
		if all_idle {
			if let Some(last_y) = nat_last_y_sent {
				if nat_received.1 == last_y {
					break;
				}
			}
			nat_last_y_sent = Some(nat_received.1);
			computers[0].inputs.push_back(nat_received.0);
			computers[0].inputs.push_back(nat_received.1);
		}
	}

	println!("{}", nat_last_y_sent.unwrap());
}
