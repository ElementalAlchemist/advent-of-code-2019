...
fn main() {
	...
	let mut lowest_x: i32 = i32::MAX;
	let mut highest_x: i32 = i32::MIN;
	let mut lowest_y: i32 = i32::MAX;
	let mut highest_y: i32 = i32::MIN;
	...
	for y in (lowest_y..=highest_y).rev() {
		for x in lowest_x..=highest_x {
			...
			let print_char = if white_tiles.contains(&coord) { '#' } else { ' ' };
			print!("{}", print_char);
		}
		println!();
	}
